...
const MangaDetailPage: React.FC = () => {
  ...
  const handleResetHistory = async () => {
    if (!manga || !window.confirm('Are you sure you want to reset the reading history for this manga?')) return;
    try {
      await apiRoutes.delete(`/history/manga/${manga.id}`);
      setUserHistories([]);
    } catch (error) {
      console.error('Gagal mereset history', error);
    }
  };
  ...
  if (!storageUrl) {
    throw new Error('VITE_STORAGE_URL is not defined in the environment. Please set it in .env');
  }
  ...
  return (
    <>
      <div className="container mx-auto px-4 py-8">
        ...
        {isAuthenticated && userHistories && userHistories.length > 0 && (
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 mb-8 shadow-lg border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-slate-800 dark:text-white">Terakhir Dibaca</h2>
              <button
                onClick={handleResetHistory}
                className="bg-blue-700 hover:bg-blue-800 text-white text-xs font-semibold py-1 px-3 rounded-md transition-colors duration-200"
              >
                Reset history
              </button>
            </div>
