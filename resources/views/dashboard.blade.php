...
<div class="container mx-auto px-4 py-8 md:px-6 md:py-10">
    @if($popularMangas->isNotEmpty())
        <div class="mb-12">
            <h2 class="text-2xl font-bold mb-6 text-gray-100">Manga Populer</h2>
                    ...
                    <div>
                        <div class="relative group">
                            <a href="{{ route('manga.show', $manga->slug) }}">
                                <div class="relative aspect-[3/4] rounded-md overflow-hidden shadow-lg">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('manga.show', $manga->slug) }}">
                                <h3 class="font-bold text-base  leading-tight truncate hover:text-blue-400 transition-colors" title="{{ $manga->title }}">
                                    {{ $manga->title }}
                                </h3>
                            </a>
                            
                            @if($manga->latestPublishedChapter)
                            <a href="{{ route('chapter.show', $manga->latestPublishedChapter->slug ) }}" class="hover:text-blue-400 transition-colors">
                            ...
                            @else
                                <p class="mt-2 text-sm  italic">Belum ada chapter</p>
                            @endif
    ...
    @if($mangas->count() > 0)
        <div>
            <h2 class="text-2xl font-bold mb-6">Update Terbaru</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-5 gap-y-8">
    ...
    @else
        <div class="text-center py-20">
            <svg class="w-20 h-20 mx-auto mb-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
            <h3 class="text-xl font-medium mb-2">Belum ada manga</h3>
            <p class="text-sm">Manga akan muncul di sini setelah ditambahkan.</p>
        </div>
    @endif
</div>
@endsection